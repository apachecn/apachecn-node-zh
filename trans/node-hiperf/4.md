...
这些可以与不同的目标一起使用，例如查看内存使用和泄漏。例如，您可以创建一个火焰图来查看中央处理器是如何使用的(繁忙的中央处理器是一个努力工作、不间断的中央处理器。空闲的中央处理器是无所事事的)。另一个很好的用途是查看应用何时空闲，与 CPU 和内存相比，输入/输出是否非常慢，当应用阻止(停止)等待来自磁盘或网络的文件时，这是正常的。这叫做脱离 CPU。这在冷色(蓝色和绿色)中更容易看到。两个 CPU 火焰图的混合还可以让您很好地理解应用的行为。
