...
5.  环回 CLI现在已经生成了我们的应用。它还自动安装了我们的`npm`依赖项。导航到应用目录并使用以下命令启动应用：
...
2.  首先向应用添加两个路由。我们将添加的第一个路由是一个`/inventory`路由，它将接受 HTTP GET 请求。此路由将使用库存中的书籍列表和可用于将书籍添加到库存的 HTML 表单进行响应。第二条路由将在`/inventory/add`端点上接受 HTTP POST 请求。`/inventory/add`路线将与书店库存微服务交互，以保存新书。将以下内容添加到`routes/inventory.js`t以创建这两条路由：
...
3.  如果我们再次检查我们的终端，我们可以看到我们的 web 应用 exp经历了一次未经处理的拒绝：
...
创建 Docker 映像时，使其尽可能小很重要。生产映像只包含在生产环境中运行应用所需的依赖项和库，这被认为是良好的做法。为了打造更小的形象，我们可以利用Docker 的多级构建功能（[https://docs.docker.com/develop/develop-images/multistage-build/](https://docs.docker.com/develop/develop-images/multistage-build/) 。
...
Kubernetes 提供了一个web UI，它提供了一个与 Kubernetes 集群和部署的应用交互的界面。默认情况下不安装 web UI，但我们可以通过以下步骤安装和访问它：
