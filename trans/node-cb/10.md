...
9.  正如我们在本章的*解释火焰图*配方中所了解到的，较暗/较粉红的边框可能表示我们应用中的瓶颈。`deserializeObject()`功能似乎是最热门的，这意味着它在 CPU 上花费的时间最多。这是 MongoDB应用中常见的瓶颈。`deserializeObject()`中的瓶颈与我们从 MongoDB 实例查询和接收的大量数据有关。
