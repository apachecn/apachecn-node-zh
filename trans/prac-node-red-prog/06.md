...
现在，您可以在 IBM Cloud 上的 Node-RED 中创建流。 Node-RED 流编辑器总是作为 Node.js 应用在 IBM云上运行。 这意味着 Node.js 运行时服务(实例)在 IBM Cloud 上是启用的。 换句话说，与运行在树莓派上的 Node-RED 不同，这个版本的 Node-RED 通过互联网访问流编辑器。
