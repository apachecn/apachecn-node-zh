...
应用是在本地主机上作为Node.js 应用直接实现的。 我们将在后面的步骤中讨论这个问题，在访问 Node-RED 运行的 localhost 端口时，将它设置为重定向到 localhost Node.js 应用。

在开始实践示例之前，我们应该了解这个应用使用的两个框架。 在本教程中，我们将使用 Node-RED 创建 ToDo 应用。 应用是通过这两个Node.js 框架实现的:
