...
为了充分利用这一章，您应该对 JavaScript 版本 ES5 或更高版本有基本的了解，并有一些使用 JavaScript 框架构建 web 应用的经验。您还需要安装 Node 和一个JavaScript 代码编辑器，例如**Visual Studio 代码**（**VSCode**）。
...
JavaScript 编程语言由 Brendan Eich 创建，并于 1995 年添加到 Netscape浏览器中。从那时起，JavaScript 获得了巨大的成功，现在也被用于构建服务器和桌面应用。然而，这种流行性和普遍性已经证明是一个问题，也是一个好处。随着越来越大的应用的创建，开发人员开始注意到这种语言的局限性。
