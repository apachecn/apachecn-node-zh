...
**路由**是 ExpressJS 核心路由系统的一个实例。这意味着 ExpressJS 应用程序中的所有路由方法都可用：
...
在这个配方中，我们将看到如何使用路由来制作模块化应用程序。开始之前，创建一个新的`package.json`文件，包含以下内容：
...
3.  为您的迷你应用程序定义路由，并添加请求方法来处理路径`"/home"`的请求：
...
4.  将模块化迷你应用程序装载到`"/first"`路径和`"/second"`路径：
...
可以看出，*路由*安装在两个不同的安装点上。路由通常被称为迷你应用程序，因为它们可以装载到 ExpressJS 应用程序的特定路由，并且不仅可以装载一次，还可以装载多次到不同的装载点、路径或 URI。
...
使用 ExpressJS 管理虚拟域非常简单。假设您有两个或更多的子域，并且希望为两个不同的 web 应用程序提供服务。但是，您不希望为每个子域创建不同的 web 服务器应用程序。在这种情况下，ExpressJS 允许开发人员使用**vhost**[在单个 web 服务器应用程序中管理虚拟域 https://expressjs.com/en/resources/middleware/vhost.html](https://expressjs.com/en/resources/middleware/vhost.html) 。
...
使用**路由**构建两个小型应用程序，将在两个不同的子域中提供服务：
...
6.  将路由安装到 ExpressJS 应用程序。在`localhost`项下送达第一份申请，在`second.localhost`项下送达第二份申请：
