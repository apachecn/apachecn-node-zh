...
有了这段代码，我们现在可以按如下方式运行*Notes*应用程序：
...
现在，我们可以在*Notes*应用程序中编写使用该数据库的代码。
...
现在我们可以使用 Sequelize 运行*Notes*应用程序了。我们可以对 SQLite3 和 MySQL 运行这个，但是让我们从 SQLite 开始。将此条目添加到`package.json`中的`scripts`条目：
