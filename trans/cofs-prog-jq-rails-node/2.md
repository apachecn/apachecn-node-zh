...
CoffeeScript可以很容易地安装在苹果电脑、视窗或 Linux 上。有多种方法可以让它运行，这取决于您是希望安装简单明了，还是希望处于危险边缘。在我们开始讨论细节之前，很高兴知道 CoffeeScript 通常不会独立运行，而是使用一些很棒的 JavaScript 工具和框架来发挥它的魔力。让我们简单讨论一下典型的CoffeeScript堆栈。

# CoffeeScript堆栈

在CoffeeScript的历史早期，它的编译器是用 Ruby 编写的。后来变成了自办；语言编译器是自己编写的。这意味着CoffeeScript的编译器是用CoffeeScript代码编写的，然后可以编译成 JavaScript，然后运行 JavaScript 再次编译CoffeeScript。很困惑，不是吗？

没有深入探讨这是什么壮举，这也意味着为了运行CoffeeScript，我们需要能够在您的计算机上独立执行 JavaScript，而不需要浏览器。

**Node.js** 、或者仅仅是 **Node** 、是一个为编写网络服务器应用而设计的 JavaScript 框架。它是使用谷歌的 V8 构建的，这是一个无需网络浏览器就能运行 JavaScript 的引擎——非常适合CoffeeScript。它已经成为安装 CoffeeScript 的首选方式。
...
核心的CoffeeScript编译器不依赖于Node，技术上可以在任何 JavaScript 环境中执行。然而，使用编译器的 coffee 命令行实用程序是一个 Node.js 包。

CoffeeScript编译器的工作如下图所示:
...
这将下载一个`.msi`文件。一旦你下载了它，运行安装再容易不过了；接受条款并点击**继续**。如果您看到以下屏幕，则表明您已成功安装了Node:
...
在这里，我使用了 **-g** 标志，为所有用户安装 npm 包。一旦您安装了CoffeeScript，我们可以使用**咖啡**命令对其进行测试，如下所示:

![Node.js, npm, and CoffeeScript on Windows](img/9588OS_02_06.jpg)

这是CoffeeScript解释器，正如你所看到的，你可以用它来运行CoffeeScript代码。要退出，只需使用 *Ctrl* + *C* 。

就这样！在 Windows 上安装 Node.js 真的是又快又简单。

# 在苹果电脑上安装CoffeeScript

在 Mac 上安装 Node.js 有两种方式，或者从 Node.js 网站下载`.pkg`文件，使用苹果的安装程序安装，或者使用**自制**命令行包管理器安装。

启动并运行最简单的方法是只安装`.pkg`文件，所以让我们先回顾一下。安装自制程序可能需要更多的工作，但是如果您更喜欢使用命令行工具并从源代码构建CoffeeScript，那么这是值得的。
...
如果您看到屏幕后面的，那么您已经成功安装了Node:

![Using the Apple installer](img/9588OS_02_08.jpg)

您还将安装 npm，我们将使用它来安装 CoffeeScript。跳到*用 npm 安装CoffeeScript*部分。
...
就这样！在苹果电脑上安装CoffeeScript非常容易。

# 在 Linux 上安装CoffeeScript
...
下面的截图显示了如何使用 **-v** 命令来安装CoffeeScript:

![Installing CoffeeScript with npm](img/9588OS_02_12.jpg)

就这样！在 Linux 上安装CoffeeScript非常容易。
...
一旦具备了这些先决条件，就应该能够构建Node了。在命令行工具上输入以下命令:
...
虽然可以在 Windows 上构建 Node.js，我强烈建议您只运行安装程序。在我在本书中提到的所有安装方式中，这是唯一一个我自己没有做过的。这个例子直接来自Node维基([https://github.com/joyent/node/wiki/Installation](https://github.com/joyent/node/wiki/Installation))。显然，构建可能需要很长时间。在命令提示符下，输入以下内容:
...
这个命令行工具就像一把CoffeeScript的瑞士军刀。您可以使用它以交互方式运行 CoffeeScript，将 CoffeeScript 文件编译成 JavaScript 文件，执行`.coffee`文件，观察文件或目录，以及编译任何文件是否发生变化，以及一些其他有用的东西。执行命令很容易，只需输入`coffee`以及它们的一些选项和参数。

有关所有可用选项的帮助，请使用 **-h** 或 **- help** 选项运行`coffee`。下面的屏幕截图显示了一系列有用的选项:

![The coffee command](img/9588OS_02_13.jpg)

我们已经看到**-v**选项，它将打印出当前版本的CoffeeScript。

## REPL

在没有参数或 **-i** 选项的情况下执行`coffee`将使您进入CoffeeScript**读取评估打印循环** ( **REPL** ) 。从这里，您可以输入将被动态执行的CoffeeScript代码，并在控制台中显示其输出。这对于使用这种语言非常有用，可以探索一些核心的 JavaScript 和 Node.js 库，甚至可以引入另一个外部库或 API，并能够交互地探索它。
...
在 REPL 中输入足够的代码后，您将会想要开始存储并在源文件中组织您的CoffeeScript。CoffeeScript文件使用`.coffee`扩展名。您可以通过将文件作为参数传递给`coffee`命令来运行`.coffee`文件。文件中的 CoffeeScript 将被编译成 JavaScript，然后使用 Node.js 作为其环境来执行。

### 类型

你可以使用任何文本编辑器来编写你的CoffeeScript。许多流行的编辑器都有插件或者增加了对 CoffeeScript 的支持，具有语法高亮、代码补全等功能，甚至允许您直接从编辑器中运行代码。在[https://github . com/jashkenas/coffee-script/wiki/Text-editor-plugins](https://github.com/jashkenas/coffee-script/wiki/Text-editor-plugins)上有一个支持 CoffeeScript 的文本编辑器和插件的综合列表。

## 编译成 JavaScript

要将CoffeeScript编译成 JavaScript，我们需要传递 **-c** 或 **-编译**选项。它要么采用带有文件名的单个参数，要么采用文件夹名称，要么采用多个文件和文件夹名称。如果您指定一个文件夹，它将编译该文件夹中的所有文件。默认情况下，JavaScript 输出文件将与源文件同名，因此`foo.coffee`将编译为`foo.js`。
...
如果你正在开发一个CoffeeScript应用，继续运行 **-编译**会变得很乏味。另一个有用的选项是 **-w** 或 **-手表**。这告诉 CoffeeScript 编译器继续运行，并监视某个文件或文件夹中文件的任何更改。这与 **-编译**结合使用效果很好，每次文件更改时都会编译。

## 把所有的放在一起

`coffee`命令很酷的一点是，这些标志可以组合起来创建一个非常有用的构建和开发环境。比方说，我在一个源文件夹中有一堆CoffeeScript文件，每次文件更改时，我都想将其编译成`js`文件夹中的一个`output.js`文件。
...
在本章中，您有望了解如何让 CoffeeScript 在您选择的开发环境中运行。您还学习了如何使用`coffee`命令运行和编译CoffeeScript。现在您有了工具，我们将开始编写一些代码，并开始了解CoffeeScript“在野外”。让我们从 JavaScript 开始的地方开始，看看在浏览器中编程 CoffeeScript。