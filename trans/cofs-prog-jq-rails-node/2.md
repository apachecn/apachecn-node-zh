...
**Node.js** 、或者仅仅是 **Node** 、是一个为编写网络服务器应用而设计的 JavaScript 框架。它是使用谷歌的 V8 构建的，这是一个无需网络浏览器就能运行 JavaScript 的引擎——非常适合咖啡脚本。它已经成为安装 CoffeeScript 的首选方式。
...
如果你正在开发一个咖啡脚本应用，继续运行 **-编译**会变得很乏味。另一个有用的选项是 **-w** 或 **-手表**。这告诉 CoffeeScript 编译器继续运行，并监视某个文件或文件夹中文件的任何更改。这与 **-编译**结合使用效果很好，每次文件更改时都会编译。
