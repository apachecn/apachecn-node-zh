...
![](assets/03b442bd-b85f-4b44-b678-59376c2e67dc.png)

下面的截图是来自该网站的图片详情页面:

![](assets/583645f2-a488-46a8-91cd-ae0421631c8e.png)
...
![](assets/21df5552-efca-45c1-9090-21550c23cac5.png)
...
路由的第一个参数是路由本身的字符串值，它可以包含变量值作为子路径。 您可以看到第二个`router.get`，我们分配了一个路由值`/images/:image_id`，它基本上等同于浏览器地址栏中的`/image/ANYVALUE`。 在编写`image.index`控制器时，您将看到如何检索`:image_id`的值并在`controller`函数本身中使用它。
