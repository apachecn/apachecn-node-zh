...
在了解了 JavaScript 的基础知识之后，让我们关注 Node 的一些基础知识。 我们将从理解 node.js 的核心架构开始。 不同Node特性的意义在于其架构和工作方式。 让我们在下一节中仔细研究一下。
...
npm 的一个真正强大的特性是，它允许其他开发人员以一种快速、简单和一致的方式在他们的本地环境中启动你的代码。 通常，Node项目包含一个名为`package.json`的特殊文件，该文件包含了关于该项目的信息，以及该项目所依赖的所有 npm 包的列表。 带有本地代码副本的开发人员可以简单地执行`npm install`，使用这个文件在本地下载和安装每个依赖项。
...
此外，还有一个很棒的网站，教 Node 使用你必须解决的实际编程问题。 这些练习的重点是理解 Node 如何工作的具体细节，并深入了解处理流、异步 I/O、承诺等的基础知识。 Node学校可在[http://nodeschool.io](http://nodeschool.io)找到。
