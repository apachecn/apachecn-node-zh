...
如果你检查在Mongoose谷歌文档或做一个快速搜索,你会发现连接 MongoDB 实例通常的方法是通过使用`'mongodb://localhost/test'``.connect()`的字符串作为参数法在Mongoose(甚至在节点 MongoDB 原生客户端)。
...
**注:**将对象的 ID 存储在名为`_id`的属性中，以下划线开头，这在使用 Mongoose 时是一个有用的约定; 它将使以后依赖于Mongoose`.findById()`模型方法成为可能。
...
一个值得注意的新事物是`entry`属性。 它将用于存储对每个注释所属条目的引用。 `ref`选项告诉Mongoose在种群中使用哪个模型，在我们的例子中是`Entry`模型。 我们在这里存储的所有`_id`都需要是 Entry 模型中的`_id`文档。
