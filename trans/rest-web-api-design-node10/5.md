...
| 方法 | URI | 媒体类型 | 描述 | 状态码 |
| 收到 | /目录 | 应用/json | 返回目录中的所有项。 | 200 行500 内部服务器错误 |
...
| 邮递 | /项目/ | 应用/json | 创建一个新项目；如果存在具有相同标识符的项，则将对其进行更新。创建项目时，返回一个**位置**头。它提供可以访问新创建的项目的 URL。 | 201 创建200 行500 内部服务器错误 |
| 放 | /item/{itemId} | 应用/json | 更新现有项目；如果具有所提供标识符的项不存在，它将创建该项。创建项目时，返回一个**位置**头。它提供可以访问新创建的项目的 URL。 | 201 创建200 行500 内部服务器错误 |
| 删去 | /item/{itemId} | 应用/json | 删除现有项目；如果具有所提供标识符的项不存在，则返回 404。 | 200 好,，404 找不到500 内部服务器错误 |
