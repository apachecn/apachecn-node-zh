...
`Redux`不是唯一可以与`React`一起使用的状态管理包。最受欢迎的当然是`MobX`，它添加了React式编程概念，如可观察对象和数组；请点击[查看 https://github.com/mobxjs/mobx](https://github.com/mobxjs/mobx) 。它的基本模式与`Redux`模式大不相同，在许多方面更简单，更类似于电子表格；然而，在使用它之前，要准备好改变你的思维方式！
...
这听起来有点神秘，所以让我们深入了解一下它是如何工作的，我们在[第 6 章](06.html)*中*定义组件*一节中构建了一个国家/地区组件**与 React*一起开发，只是这次我们将处理实际的 API 调用，我们已经有了我们在[第 4 章](04.html)中创建的`Node`服务器，*通过Node*实现 RESTful 服务。
...
*   然后，我们使用`axios()`包，正如前面在Node工作中使用的一样，调用服务器并获取国家列表。
...
我们前面的路由示例运行得很好，但在某些应用中，您可能需要授权，以便只有登录的用户才能访问您网站的部分内容。（如果您使用的是我们在[第 4 章](04.html)中开发的 API，*通过Node*实现 RESTful 服务，而Node需要**JSON Web 令牌**（**JWT**），那么您还需要识别用户.那么，让我们看看我们需要做哪些额外工作才能在页面上同时显示受限和非受限路由。
