...
最好从 web 设计的这些静态方面开始，然后再处理动态方面，如对事件的React或获取数据。让我们来看看代码。
...
| `React.Element<typeof Component>` | 特定类型的Node：例如，`React.Element<"div">`是呈现`<div>`的元素。 |
| `React.Key` | 用作键的道具类型：本质上是数字或字符串。 |
| `React.Node` | 可以呈现的任何Node，包括 React 元素、数字、字符串、布尔值、未定义、null 或这些类型的数组。 |
