...
在前一章，我们安装并介绍了Mocha。Mocha 是一个运行在 Node.js 内部或者浏览器内部的 JavaScript 测试框架。您可以使用它来定义和运行您自己的测试。Mocha报告了结果:哪些测试运行良好，哪些测试失败，哪里出现了任何失败。Mocha依次运行每个测试，等待一个测试完成或超时后再运行下一个测试。

即使 Mocha 被设计成能够在任何现代浏览器上运行，我们也只能通过命令行通过 Node.js 来运行它。Mocha还可以做其他事情，这一章会解释。有关Mocha功能的更完整参考，请访问Mocha官方文档网站、[http://visionmedia.github.com/mocha/](http://visionmedia.github.com/mocha/)了解更多信息。
...
将该文件代码保存到名为`test.js`的文件中，并在本地安装Mocha:
...
我们已经看到，在 描述范围内，可以有各自的`before`、`after`、`beforeEach`和`afterEach`钩子。如果您有一个嵌套的`describe`范围，该范围也可以有钩子。除了当前作用域上的钩子，Mocha还将调用所有父作用域上的钩子。考虑一下这段代码，其中我们声明了一个两级嵌套:
...
Mocha串联运行所有测试，每个测试可以是同步的，也可以是异步的。对于同步测试，测试回调函数不应该接受任何参数，就像前面的例子一样。但是由于 Node.js 不阻止 I/O 操作，并且我们需要为我们的每个测试执行 I/O 操作(至少向我们的服务器发出一个 HTTP 请求)，所以我们的测试需要是异步的。
...
**超时**:默认情况下，Mocha每次异步测试预留 2 秒。您可以通过将`-t`参数传递给Mocha来进行全局更改:
...
Mocha是一个运行你的测试的框架。您应该根据您想要覆盖的功能区域将您的测试分成几个文件，然后描述每个特性并为每个特性定义必要的测试。
...
对于异步测试，Mocha保留 2 秒的默认超时时间，您可以全局覆盖或按测试覆盖。

在下一章中，我们将看到如何开始使用Zombie.js 来模拟和操作浏览器。