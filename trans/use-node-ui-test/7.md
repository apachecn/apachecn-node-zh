...
默认情况下，Zombie不会将内部事件输出到控制台，但是您可以在`DEBUG`环境变量设置为`true`的情况下运行Zombie。如果您有一个 UNIX shell 命令行，您可以在测试套件的启动前加上`DEBUG=true`，如下所示:
...
通过启用这个环境变量，Zombie将输出它发出的每个 HTTP 请求，以及收到的 HTTP 状态代码:
...
如你所见，Zombie也报告了所有的`3xx-class` HTTP 重定向和新的网址是什么。

该输出可能有助于调试一些网址加载问题，但很难跟踪特定的 HTTP 请求指的是哪个测试。

幸运的是，有一种方法可以通过改变Mocha记者来给测试输出带来一些澄清。Mocha自带一个叫记者的功能。到目前为止，我们已经使用了默认报告器，它为每个测试报告一个彩色点。但是如果指定`spec`报告者，Mocha会在测试开始前和测试结束后输出测试名称。

要启用 `spec`记者，只需将`-R spec`添加到Mocha参数中，如下所示:
...
如果您在某个特定测试中遇到困难，您不需要运行整个测试套件甚至整个测试文件。Mocha接受一个 `-g <expression>`命令行选项，并且只运行与该表达式匹配的测试。
...
在这里可以看到如预期的那样，Zombie只输出名为`should allow you to remove`的测试的调试信息。

# 使用浏览器 JavaScript 控制台

除了浏览器发出的 HTTP 请求之外，Zombie在调试时不会输出太多其他有趣或有用的东西来帮助您调试应用。

一个很好的选择是使用开发工具和/或调试器在真实的浏览器中运行您的应用，它提供了更多的灵活性和洞察力。

调试Zombie工具遇到的问题的一个特别有用的替代方法是在浏览器代码中使用`console.log()`函数(在这个应用中，这个代码位于`public/js`目录中)。
...
浏览器开发工具更适合调试浏览器应用。但是，如果您遇到特定于Zombie的问题，有几种技术可能会帮助您。

一个是启用Zombie调试输出。这将向您显示浏览器正在加载哪些资源，以及旁边显示的相应响应状态代码。

您可以运行特定的测试。调试测试中的特定问题时，也可以使用`-g <pattern>`选项限制Mocha只运行该测试。
