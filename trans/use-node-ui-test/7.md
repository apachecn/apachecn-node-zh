...
除了浏览器发出的 HTTP 请求之外，僵尸在调试时不会输出太多其他有趣或有用的东西来帮助您调试应用。

一个很好的选择是使用开发工具和/或调试器在真实的浏览器中运行您的应用，它提供了更多的灵活性和洞察力。

调试僵尸工具遇到的问题的一个特别有用的替代方法是在浏览器代码中使用`console.log()`函数(在这个应用中，这个代码位于`public/js`目录中)。
...
浏览器开发工具更适合调试浏览器应用。但是，如果您遇到特定于僵尸的问题，有几种技术可能会帮助您。
