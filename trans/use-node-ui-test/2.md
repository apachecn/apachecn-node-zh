...
在我们深入Zombie.js 世界之前，我们需要为我们的测试创建一个目标，也就是一个提供待办事项列表的 web 应用。这是此类应用的一组顶级要求:
...
如果您没有安装最新版本的 Node.js，您将需要安装它。出于几个原因，您将需要 Node.js。我们的 web 应用将使用 Flatiron，它运行在 Node.js 之上。您还需要使用与 Node 捆绑在一起的**节点包管理器**(**【NPM】**)。最后，您将需要 Node.js 来安装和运行您的Zombie.js 测试。
