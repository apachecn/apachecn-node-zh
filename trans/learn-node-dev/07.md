...
现在有可能事情进展不顺利。我们必须处理Node应用中的错误。在这种情况下，我们不会调用`resolve`，而是调用`reject`。让我们注释掉`resolve`行，然后创建第二行，我们称之为`reject`。我们将调用`reject`的方式与调用`resolve`的方式大致相同。我们必须传入一个参数，在这种情况下，像`Unable to fulfill promise`这样的简单错误消息可以：
...
如果是这样的话，那么事情就糟了，我们不想继续提出天气要求。我们想运行我们的捕获代码。要抛出承诺可能捕获的新错误，我们将使用名为`throw new Error`的语法。这将创建并抛出一个错误，让Node知道发生了错误。我们可以提供我们自己的错误消息，这是用户可读的：`Unable to find that address`：
...
在下一章中，我们将开始创建异步的 web 服务器。我们将制作异步的 API。此外，我们还将创建实时 Socket.IO 应用，它将是异步的。我们将继续创建部署到服务器上的Node应用，使任何具有 web 连接的人都可以访问这些服务器。