...
在本章中，我们将继续讨论更多的Node基础知识。我们将探索 yargs，并将了解如何使用`process.argv`和 yargs 解析命令行参数。之后，我们将探索 JSON。JSON 只不过是一个看起来有点像 JavaScript 对象的字符串，其显著区别在于它使用双引号而不是单引号，并且所有属性名（如`name`和`age`）都需要在其周围加引号。我们将研究如何将对象转换为字符串，然后定义该字符串，使用它，并将其转换回对象。
...
在本节中，我们将使用 yargs（第三方 npm 模块）来简化解析过程。它将允许我们访问诸如标题和正文信息之类的内容，而无需编写手动解析器。这是一个很好的例子，说明了什么时候应该寻找 npm 模块。如果我们不使用模块，那么Node应用使用经过测试和彻底审查的第三方模块将更有效率。
...
到目前为止，yargs 还没有发光。这并不比上一个示例中的有用多少。当我们开始以键值对进行传递时，这才是真正的亮点，例如我们在[第 2 章](02.html)中*Node基础知识第 1 部分*的*获取输入*部分中使用的标题示例。我可以将我的`title`标志设置为`secrets`，按下*回车*，这一次，我们得到了更有用的东西：
...
这就是我们调试错误消息的方法。错误消息包含非常有用的信息。在大多数情况下，前几行是您编写的代码，其他几行是内部Node代码或第三方模块。在我们的例子中，堆栈跟踪的第一行很重要，因为它准确地显示了错误发生的位置。
...
在`addNote`函数上方，让我们加载`fs`模块。我将创建一个名为`fs`的`const`变量，并将其设置为等于`require`的返回结果，我们将需要`fs`模块，它是一个核心Node模块，因此无需使用 NPM 进行安装：
...
在下一章中，我们将继续学习Node基础知识。我们将探讨更多与Node相关的概念，例如调试；我们将处理`read`和`remove`notes 命令。除此之外，我们还将了解 yargs 的高级功能和箭头功能。