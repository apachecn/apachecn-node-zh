...
ExpressJS 的路由方法通常充当控制器。他们只是从客户端获取输入，例如从浏览器获取请求，然后将输入转换为操作。然后，这些操作被发送到模型（即应用的业务逻辑，例如 mongoose 模型）或视图（ReactJS 客户端应用）进行更新。
...
我们的 RESTful API 应用将在端口`1337`上本地运行。当运行测试代码时，它将连接到它并使用不同的 HTTP 方法发出多个请求，以创建用户、检索用户、更新用户和删除用户。所有操作都将记录在终端中。
...
*   [第 1 章](1.html)、*MERN 栈简介*、*节安装 NPM 包*
...
2.  包括所需的库。然后，初始化新的 ExpressJS 应用并创建到 MongoDB 的连接：
...
4.  定义 ExpressJS 中间件功能，确保您的 web 应用在允许执行下一个路由处理程序之前先连接到 MongoDB：
...
然后，创建一个名为`api`的新目录。接下来，创建应用的模型或业务逻辑。使用静态和实例方法为用户定义架构，这些方法将允许用户注册、登录、注销、获取配置文件数据、更改密码和删除配置文件：
...
在另一个终端中，运行刚才创建的 NodeJS REPL 应用：
